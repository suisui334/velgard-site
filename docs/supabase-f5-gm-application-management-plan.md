...
## 13. F-5 dev確認後の本番UX方針

F-5 devプロトタイプは、GMが自分のsession申請を `accepted` / `rejected` へ変更できること、player / 未ログインが管理操作できないこと、参加人数RPCを再読込できることをユーザー実ブラウザで確認済み。

ただし、F-5 dev画面は本番UIの完成形ではなく、権限・RPC・RLS・状態変更の検証足場である。

本番統合時は、専用GM一覧ページを前面に出すより、`session-detail.html` の参加希望コメント欄へ統合する方針を優先する。

- コメント投稿は参加申請の意思表示として扱う
- 参加人数はコメント件数ではなく `session_applications` の `session_id + user_id` 単位で数える
- GMのみ、選択中セッションのコメント一覧上で承認・却下などの管理操作が見える
- PLには自分の投稿、申請状態、必要な案内だけを見せる
- コメント編集・削除、申請取消、承認済み取り消しは後続設計で扱う

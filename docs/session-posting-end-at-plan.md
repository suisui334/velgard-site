...
フォーム側は `終了日時` から `p_end_at` を送信する実装へ切り替えた。
`p_end_at` は `YYYY-MM-DD HH:mm` 形式で送り、互換用に `p_end_time` も送る。
日跨ぎ終了日時の投稿前ブロックは解除した。
終了日時が開始日時以下の場合は `終了日時は開始日時より後にしてください。` として投稿前に止める。

表示側はSupabase `end_at` を `endAt` に正規化し、`endAt` があれば終了日時として優先表示する。
`endAt` がない既存データは従来の `endTime` にフォールバックする。
`レベル帯`、`依頼書本文`、`参加条件` 欄はフォームへ復活させていない。
...
- `終了日時` から `p_end_at` を送る。M-14D-5で実装済み。
- 互換表示用として `p_end_time` も終了日時の時刻部分を送ってよい。
- 日跨ぎ終了日時の投稿前ブロックは解除する。M-14D-5で解除済み。
- 終了日時が開始日時以下の場合は投稿前に拒否する。

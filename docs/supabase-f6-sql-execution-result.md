...
F-6 RLS smoke test更新計画は `docs/supabase-f6-rls-smoke-test-update-plan.md` に分離する。
...
F-6 devコメント編集・削除プロトタイプ設計は `docs/supabase-f6-comment-edit-delete-prototype-plan.md` に分離する。
この設計では、dev配下の画面構成、編集・削除操作、確認ダイアログ、状態変更テストの注意、本番統合前条件を整理する。

F-6 devコメント編集・削除プロトタイプは `dev/supabase-comment-edit-delete-prototype.html` / `dev/supabase-comment-edit-delete-prototype.js` に分離する。
実装メモは `docs/supabase-f6-comment-edit-delete-prototype.md` に記録し、本番ページ接続なしで `update_application_comment` / `delete_application_comment_and_maybe_cancel` を検証する。

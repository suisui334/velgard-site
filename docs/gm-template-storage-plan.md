...
2. M-15K候補: mypageテンプレート管理UIの設計。自由本文系とJSON系の扱いを分ける。
3. M-15L候補: mypageで自由本文系テンプレートの一覧、作成、編集、削除をRPC経由で接続する。
4. M-15M候補: session-detailのPL参加希望コメント欄に `application` / `other` テンプレート呼び出しを追加する。
5. M-15N候補: mypageで `session_post` テンプレートの一覧と削除、必要ならフォーム風編集UIを追加する。
6. M-15O候補: `other` の混線が残る場合、利用文脈の追加設計をpreflightから検討する。

...
2. publishable key / anon keyをrepoへ置くかどうかのユーザー最終判断
3. 未構成フォールバック維持のままサイト側軽作業へ戻る
4. 実config運用案を決めたうえで、M-5 Auth client初期化計画を作る
5. 実接続へ進む場合も、まず `mypage.html` 内のログイン / ログアウト入口に限定する

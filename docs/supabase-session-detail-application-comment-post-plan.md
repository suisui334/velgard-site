...
| M-11B-2 | バリデーション、二重押し防止、`create_application_comment` 呼び出し、短い成功/失敗表示、投稿成功後のコメント一覧・人数・本人申請状態の再取得。楽観表示なし | 投稿時のみあり |
| M-11B-3 | 必要になった場合の投稿UI微調整、ユーザー実ブラウザ確認結果の反映 | なし想定 |

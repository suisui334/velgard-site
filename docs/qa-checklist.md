...
- [x] `session-detail.html` の表示順がPL向けに、基本情報、概要、詳細 / 参加条件、参加希望コメント、タグ、補足情報の順へ整理されている
- [x] `session-detail.html` 上部に開催日、開催時刻、GM、レベル帯、募集人数がまとまって表示される
- [x] `session-detail.html` 下部の補足情報が、状態と更新日時中心に整理されている
- [x] PL向け詳細に関連スポットID、シナリオID、公開範囲が表示されない
- [x] `updatedAt` が時刻付きの場合、`2026-05-29T21:30:00` から `2026-05-29 21:30` のように表示される
- [x] `updatedAt` が日付のみでも表示が壊れない
...
- [x] 通常予定の `session-detail.html` に「参加希望コメント」静的UIモックが表示される
- [x] 通常予定で disabled のテンプレート選択UIが表示される
- [x] 通常予定で disabled の textarea が表示される
- [x] 通常予定で disabled の `コメント投稿（準備中）` ボタンが表示される
- [x] 旧 `参加申請する（準備中）` ボタンが表示されない
- [x] `closed` 予定で `募集締切` 表示になり、新規参加希望コメントを受け付けない旨が表示される
- [x] `finished` / `canceled` 予定では終了・中止メッセージのみ表示され、投稿UIが出ない
- [x] 参加人数はコメント件数ではなく申請者単位で管理する旨が表示される
- [x] コメント投稿、保存、編集、申請用テンプレート保存、参加人数自動再計算は未実装のままである
- [x] 予定がない日の空表示が自然である
- [x] セッション予定表示追加後もラクシア日付、季節、月齢、レベルキャップ表示が維持されている
- [x] Phase 1-A / Phase 1-B / Phase 1-C / Phase 1-D / Phase 1-E / Phase 2-A は静的モックUIと表示整理、詳細ページ導入、日付保持、参加希望コメント準備表示であり、予定登録、編集、実コメント投稿、認証、Discord連携、外部DB/APIを実装していない
- [x] 〆ボタン実処理、参加申請停止処理、保存処理は未実装のままである
- [x] カレンダー / session-detail 関連キャッシュクエリが `v=20260529-session-comment-ui-mock` である

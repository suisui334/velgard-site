...
- [x] 選択日予定カードにタイトル、開催時刻、GM名、レベル、募集人数、概要、タグ、Discordスレッドリンクが表示される
- [x] 予定がない日の空表示が自然である
- [x] セッション予定表示追加後もラクシア日付、季節、月齢、レベルキャップ表示が維持されている
- [x] Phase 1-A は静的モックUIであり、予定登録、編集、参加申請、認証、Discord連携、外部DB/APIを実装していない
- [x] 〆ボタン、参加申請停止処理、保存処理は未実装のままである
- [x] カレンダー関連キャッシュクエリが `v=20260529-calendar-sessions-mock-3` である
- [x] galleryスワイプ用キャッシュクエリ `v=20260529-gallery-swipe` が維持されている
- [x] カレンダー予定表示は390px幅で横スクロールが出ない
- [x] 390px幅で横スクロールが出ない
- [x] raw JSON / undefined / null / [] が露出していない
- 注記: ラクシア運用カレンダー Phase 1、月表示カレンダー、期間外表示、ラクシア年切り替わり修正、レベルキャップ開始日可視化、Phase 1-A 静的セッション予定モックUIはユーザー実ブラウザ確認済み。

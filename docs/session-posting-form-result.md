...
## M-14D-7 自分の依頼書一覧の選択式化

`session-post.html` の `自分の依頼書` 一覧を、フォーム下部に長く並ぶカード列や右外側の独立パネルではなく、フォーム内の `公開状態` 欄の直下に収まるコンパクトな管理ボックスへ変更した。
一覧は募集状態、公開状態、タイトル、開催日時から終了日時だけを表示し、概要や詳細メタ情報は出さない。

行を選ぶと同ページ内のメインフォームへ既存依頼書の値を反映し、`編集中: 依頼書タイトル` の編集モードへ切り替える。
反映する値はタイトル、開始日時、終了日時、申請締切、種別、募集人数min/max、公開状態、募集状態、概要に限定する。
...
フォーム内の管理ボックスに出す詳細表示は公開状態、募集状態、Discord同期状態、作成日時、更新日時に限定する。
編集保存・公開切替・削除・募集終了は未実装。
Discord実送信も引き続き未実装。
`gm_user_id`、email、user_id全文、gmUserId、token、key、secret、Discord credential、Webhook URL、bot token、service_roleは表示しない。

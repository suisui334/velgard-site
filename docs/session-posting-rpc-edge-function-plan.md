...
## 33. M-14D-12A delete-equivalent and closed-state hints

`session-detail.html` の削除ボタンは、物理削除ではなく `update_session_post` RPC で `visibility = hidden` / `status = canceled` を保存する削除相当操作として接続した。既存のタイトル、日時、概要、募集人数、種別、締切、`end_at` は維持し、Discord実送信やEdge Function deployは行わない。

操作許可は Supabase由来かつ admin または作成者GMのみ。静的JSON由来、通常PL、他GMでは削除不可とし、画面に削除できない理由を表示する。成功時は詳細画面に `非公開` / `中止` の状態を反映し、失敗時は `login_required`、`not_allowed`、`session_not_found` を日本語に丸める。

`session-post.html` では `closed` / `finished` / `canceled` の補助文を整理し、募集終了、開催終了、中止の保存結果が分かるようにした。raw id / uuid / user_id / email / token / secret 類は画面、DOM、console、docsへ出さない方針を維持する。

この工程では SQL Editor実行、DB構造変更、RPC変更、GRANT/REVOKE、物理削除、Discord実送信、Edge Function deploy、Discord resync UI、`updates.json` 変更、service_role key利用、フロントからのDB直UPDATE、commit / push は行っていない。

## 34. M-14D-13A delete policy change
...
適用後確認では、`function_count = 1`、`all_security_definer = true`、signatureは `delete_session_post(text)`。
権限は `authenticated EXECUTEあり`、`anon EXECUTEなし`、`public EXECUTEなし` で、期待値どおり。

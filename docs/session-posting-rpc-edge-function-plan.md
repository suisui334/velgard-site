...
`public.sessions` の `session_type` / `application_deadline` / Discord同期メタデータ列、check制約、`create_session_post(...)` RPC、grantを確認済み。`create_session_post(...)` の実行テスト、Edge Function deploy、Discord実送信、フロント実装は未実施。

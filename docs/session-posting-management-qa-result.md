...
GM向け申請履歴と承認済み連絡先は、内部 `user_id` を返さないRPCがあるため、GM本人のユーザー名を使ったbest-effort除外とした。厳密に除外するには、後続でRPC側にGM本人除外条件を入れるか、内部IDを返さずにGM本人判定済みの結果だけ返す設計が必要。
...
M-15Aとして、`{{session_title}}`、`{{approved_call_list}}`、`{{approved_pc_names}}` の初期テンプレート変数に必要なPC名保存方針を整理した。新規docs `docs/player-character-registration-plan.md` に、PC名保存方式の候補、推奨方式、mypage登録方針、参加申請との紐付け、後続SQL/RPC候補、段階実装案を記録した。

PC名保存方式は、`profiles.default_pc_name` 単独案、`player_characters` テーブル案、`session_applications.pc_name_snapshot` 案を比較した。推奨は、`player_characters` でPC名を管理し、`session_applications` に `selected_character_id` と `pc_name_snapshot` を持たせる複合案。初期実装ではデフォルトPCを自動採用し、後続で参加申請時PC選択へ拡張する。

テンプレート出力では、承認済み参加者の `display_name`、`discord_handle`、`pc_name_snapshot` を使う方針とした。`{{approved_call_list}}` は `Discord：<@DiscordユーザーID>｜ユーザー名：ユーザー名｜PC名：PC名` のラベル付き1人1行、DiscordユーザーID未登録/形式不正は `Discord：登録されていません`、PC名未登録は `PC名：PC名未登録` を出す。`{{approved_pc_names}}` は承認済み参加者のPC名をまとめ、未登録は空欄にせず `PC名未登録` を含める。
...
## M-15D player_characters APPLY結果

ユーザーがSupabase SQL Editorで `docs/supabase/sql/019_player_characters_apply_reviewed.sql` を適用し、PC名登録・参加申請PC紐付け用DB変更が完了した。

確認結果として、`player_characters` table と `player_characters.id` / `owner_user_id` / `pc_name` / `is_default` / `is_active` は存在し、すべて `ok = true`。
`session_applications.selected_character_id` と `session_applications.pc_name_snapshot` も作成済みで `ok = true`。
`owner_user_id` は `profiles(id)` を参照し、`selected_character_id` は `player_characters(id)` を参照するFKとして確認済み。

PC管理RPC 5本、`create_player_character(text, boolean)`、`deactivate_player_character(uuid)`、`get_my_player_characters()`、`set_default_player_character(uuid)`、`update_player_character(uuid, text, boolean, boolean)` は作成済み。
各RPCは `security_definer = true` で、権限は `authenticated EXECUTEあり`、`anon / public EXECUTEなし`、すべて `ok = true`。
...
M-15Eとして、mypageのログイン済み画面にPC名登録UIを追加した。PC名一覧は `get_my_player_characters()` で取得し、新規登録は `create_player_character(text, boolean)`、編集は `update_player_character(uuid, text, boolean, boolean)`、既定PC設定は `set_default_player_character(uuid)`、一覧から外す操作は `deactivate_player_character(uuid)` に接続する。

PC名は空欄、改行、40文字超過を保存前に止める。未登録時は「現在、登録済みPC名はありません。」を表示し、作成 / 更新 / 既定PC設定 / 一覧から外す操作の成功メッセージと既知エラーの日本語化を行う。

「一覧から外す」は物理削除ではなく非アクティブ化で、確認文に「過去の参加申請に保存されたPC名は残ります。」を明記する。現時点では有効なPCのみを一覧表示する。

PCレコードのDB uuidはJSメモリ上でのみ保持し、DOM上は `pc-0` などのローカルキーで扱う。raw DB uuid / Supabase user_id / email / token / secret類は画面、DOM、consoleに出さない。

今回の範囲では、参加申請時のPC名スナップショット保存、承認済み参加者一覧へのPC名表示、テンプレート変数 `{{approved_call_list}}` / `{{approved_pc_names}}` の実置換は未実装で後続課題とする。
...
## M-15F 参加申請PC名スナップショット設計

M-15Fとして、参加申請コメント投稿時にPC名snapshotを保存するRPC草案とpreflight SQLを作成した。対象は `create_application_comment(target_session_id text, comment_body text)` の後続置換案で、フロントから渡す値は従来どおりセッションIDと自由本文コメントのみとする。

参加申請コメントは自由本文であり、ユーザー名、PC名、DiscordユーザーIDを手入力させない。ユーザー名は `profiles.display_name`、DiscordユーザーIDは `profiles.discord_handle`、PC名は本人の `player_characters` 既定PCから取得する。コメント本文からPC名やDiscord IDを解析しない。

新規PL申請では、既定PCがあれば `selected_character_id` / `pc_name_snapshot` を保存する。既定PCがない場合も申請は許可し、snapshotは `null` とする。辞退済みからの再申請では、その時点の既定PCでsnapshotを更新する。コメント編集時はsnapshotを維持する。

GM本人コメントは許可するが、参加申請として扱わない。RPC草案ではGMコメントを `session_comments.is_application = false` として保存し、`session_applications` 行の作成/更新やPC snapshot保存を行わない。GMコメントは参加人数、申請者一覧、承認済み連絡先、テンプレート変数対象から除外する。

M-15GではGM向け承認済み参加者一覧/連絡先表示にPC名を含め、M-15Hでは `{{session_title}}`、`{{approved_call_list}}`、`{{approved_pc_names}}` のテンプレート変数接続を行う想定。今回CodexはSQL Editor実行、DB構造変更、RPC作成 / 置換、GRANT / REVOKE実行、フロントUI実装、Discord実送信、Edge Function deploy、`updates.json` 変更、secret類の出力、commit / pushを行っていない。

## M-15D補正 selected_character_id FK確認

M-15D適用後の確認で、`session_applications_selected_character_id_fkey` が `FOREIGN KEY (selected_character_id) REFERENCES player_characters(id)` となっており、期待していた `ON DELETE SET NULL` が付いていないことを確認した。

M-15Fへ進む前に補正する方針として、`docs/supabase/sql/021_fix_selected_character_fk_preflight_select_only.sql` と `docs/supabase/sql/021_fix_selected_character_fk_apply_reviewed.sql` を作成した。preflightはSELECT-onlyで、APPLY専用SQLは既存FKをdropして `ON DELETE SET NULL` 付きで作り直し、実行後確認SELECTでdefinitionと `confdeltype` を確認する内容。
...
ユーザーが021 selected_character FK補正preflightをSupabase SQL Editorで実行し、`selected_character_fk_has_on_delete_set_null = true` を確認した。現DB上では `session_applications.selected_character_id` のFKはすでに `ON DELETE SET NULL` 相当で確認できた。

`021_fix_selected_character_fk_apply_reviewed.sql` は未実行で、現時点では実行不要とする。前回の `ON DELETE SET NULL` 不足は、表示上の見切れまたは確認不足だった可能性として扱う。DB追加変更、ALTER TABLE実行、RPC変更、GRANT / REVOKE実行は行っていない。

次はM-15Fとして、参加申請PC名スナップショット接続へ戻る。

## M-15F preflight確認結果

修正版 `020_application_pc_snapshot_preflight_select_only.sql` は、SQL Editorで `ERROR: 42809: "array_agg" is an aggregate function` により途中停止した。SELECT-only preflight中のエラーで、DB変更は起きていない。

小型確認SQLで、`public.player_characters`、`session_applications.selected_character_id`、`session_applications.pc_name_snapshot`、`create_application_comment(text,text)`、`cancel_my_session_application(text)`、`get_gm_session_accepted_contacts(text)`、`get_my_player_characters()` は存在確認済み。`session_applications.status` 許可値は `pending` / `accepted` / `rejected` / `waitlisted` / `canceled`。
...
APPLY前レビューで、adminコメントも管理コメント扱いになるよう `public.is_admin() or public.is_session_gm(v_target_session_id)` へ修正した。adminが他GMのセッションへコメントしてもPL参加申請扱いにならず、`session_applications` 作成/更新やPC snapshot保存は行わない。

コメント本文の保存値は、元の `comment_body` ではなくtrim後の `v_comment_body` に修正した。SQL Editor実行、DB構造変更、RPC作成/置換実行、GRANT / REVOKE実行は行っていない。

## M-15F application PC snapshot APPLY結果

ユーザーがSupabase SQL Editorで `docs/supabase/sql/020_application_pc_snapshot_apply_reviewed.sql` を適用し、`create_application_comment(text,text)` の置換が成功した。

確認結果は `function_count = 1`、`all_security_definer = true`、signatureは `create_application_comment(text,text)`、function configに `search_path` 設定あり。権限確認は `authenticated EXECUTEあり`、`anon EXECUTEなし`、`public EXECUTEなし` で、すべて `ok = true`。`session_applications.selected_character_id` / `pc_name_snapshot` の存在も確認済み。

PL新規申請・再申請時は既定PCをsnapshotする。PC名未登録でも申請可能。GM/admin管理コメントでは参加申請扱いせず、`selected_character_id` / `pc_name_snapshot` を保存しない。参加申請コメント本文は自由本文で、PC名やDiscordユーザーIDを本文に書かせない。

実データ投入、フロントUI変更、参加申請UI変更、Discord実送信、Edge Function deploy、`updates.json` 変更は行っていない。

## M-15F application PC snapshot 実動作確認

実ブラウザ / SQL確認で、通常PLの参加申請時に `session_applications.pc_name_snapshot` へ既定PC名が保存され、`selected_character_id` も紐付いたことを確認した。SQL確認では `linked_pc_name` と `pc_name_snapshot` が一致した。
...
## M-15G GM向けPC名付き連絡先RPC preflight結果

ユーザーがSupabase SQL Editorで `022_gm_accepted_contacts_pc_name_preflight_select_only.sql` を実行した。既存 `get_gm_session_accepted_contacts(text)` は signature `get_gm_session_accepted_contacts(text)`、arguments `target_session_id text`、result type `TABLE(display_name text, discord_handle text)` の2列返却と確認済み。`security_definer = true` で `search_path` 設定あり。
...
PC名表示には戻り値列追加が必要。既存列 `display_name` / `discord_handle` は維持し、`discord_mention` / `pc_name` / `pc_name_missing` を追加候補とする。`pc_name` は `session_applications.pc_name_snapshot` を正とし、null/空は `PC名未登録`。DiscordユーザーIDが17〜20桁の数字なら `<@ID>` に変換し、未登録/形式不正は `登録されていません` とする。GM本人を承認済み参加者一覧から除外し、raw user_id / email / token / selected_character_id / application_id は返さない。

同名RPCで戻り値型を変更する場合はdrop/recreateが必要になる可能性があるため、後続APPLYではdrop/recreate案とv2 RPC案をレビューする。今回はSQL Editor追加実行、DB構造変更、RPC作成/置換、GRANT / REVOKE、APPLY専用SQL作成、フロントUI実装、Discord実送信、Edge Function deploy、`updates.json` 変更は行っていない。

## M-15G GM向け承認済み参加者PC名表示RPC APPLY専用SQL

M-15Gとして `docs/supabase/sql/022_gm_accepted_contacts_pc_name_apply_reviewed.sql` を作成した。既存 `get_gm_session_accepted_contacts(text)` は `display_name` / `discord_handle` の2列返却だったため、戻り値型変更に備えてdrop/recreate方針を採用した。

APPLY後の戻り値は `display_name` / `discord_handle` / `discord_mention` / `pc_name` / `pc_name_missing`。既存列は維持し、PC名は `session_applications.pc_name_snapshot` を正とする。PC名未登録時は `PC名未登録`、DiscordユーザーID未登録・形式不正時は `登録されていません` とし、形式不正値、raw user_id / email / token は返さない。

対象は承認済み申請のみで、GM本人は除外する。権限方針は未ログイン拒否、GM/adminのみ、`security definer`、`set search_path = ''`、`authenticated` のみEXECUTE。実行後確認SELECTとして関数本数、signature、戻り値列、権限状態を確認する内容を含めた。

今回CodexはAPPLY未実行、SQL Editor未実行、DB構造変更なし、RPC作成/置換未実行、GRANT / REVOKE未実行、フロントUI実装なし、Discord実送信なし、Edge Function deployなし、`updates.json` 未変更。
...
確認結果は `function_count = 1`、`all_security_definer = true`、`has_search_path_config = true`、signature `get_gm_session_accepted_contacts(text)`。戻り値列 `display_name` / `discord_handle` / `discord_mention` / `pc_name` / `pc_name_missing` はすべて存在確認済み。

権限は `authenticated EXECUTEあり`、`anon EXECUTEなし`、`public EXECUTEなし` で、すべて `ok = true`。既存列 `display_name` / `discord_handle` は維持し、`pc_name` は `session_applications.pc_name_snapshot` を正とする。PC名未登録時は `PC名未登録`、DiscordユーザーID未登録・形式不正時は `登録されていません`。raw user_id / email / token は返さない。
...
## M-15G 承認済み参加者連絡先PC名表示フロント実装

session-detailのGM/admin向け承認済み参加者連絡先表示にPC名を追加した。`get_gm_session_accepted_contacts(text)` の `discord_mention` / `pc_name` / `pc_name_missing` を受け取り、画面表示は `Discord：discord_mention｜ユーザー名：display_name｜PC名：pc_name` のラベル付き1人1行とした。

コピー出力も画面表示と同じラベル付き1人1行に変更した。PC名未登録は `PC名：PC名未登録`、Discord未登録・形式不正は `Discord：登録されていません` とし、不正な `discord_handle` 生値や raw user_id / email / token は表示しない。

参加申請コメント表示、申請承認/却下、GMコメント表示、GMコメントを参加者にカウントしない方針は維持する。SQL Editor未実行、DB構造変更なし、RPC変更なし、Discord実送信なし、Edge Function deployなし、`updates.json` 未変更。

## M-14D-15 依頼書RPC/RLS smoke test整理

依頼書の作成、編集、公開状態変更、募集状態変更、中止、完全削除を横断して確認するためのsmoke test観点を `docs/session-posting-rpc-smoke-test-plan.md` に整理した。

対象RPC候補は `create_session_post(...)`、`update_session_post(...)`、`delete_session_post(text)`。未ログイン、通常PL、作成者GM、他GM、admin、静的JSON由来の文脈別に、作成 / 更新 / 完全削除 / 管理UI表示の期待値を整理した。

入力バリデーションは `draft + public`、不正な `status` / `visibility`、人数範囲、終了日時逆転、タイトル / 概要空欄、一般化したエラー表示を確認候補とする。

完全削除は `delete_session_post(text)`、中止として残す場合は `status = canceled`、募集終了 / 開催終了は募集状態変更として扱う。完全削除時は依頼書本体に加え、参加申請と参加希望コメントへの影響を確認文へ含める観点を維持する。

静的JSON由来はDB RPCの編集 / 削除対象にせず、同じ公開IDのSupabase由来がある場合はSupabase側を優先する。非公開 / 下書き / 中止のSupabase由来が静的JSON fallbackで復活しないことを後続QA候補にした。
...
## M-14D-15C 依頼書RPC smoke test 手動実行手順

依頼書RPC smoke testを実際に行う前の手動実行手順とテストデータ設計を `docs/session-posting-rpc-smoke-manual-test.md` に整理した。

推奨方式は実ブラウザ操作中心とし、SQL Editorでの直接RPC確認は必要最小限にする。テスト用依頼書は既存本番寄りデータと分け、基本確認用、完全削除用、CASCADE確認用を混ぜない方針にした。

ロール別には未ログイン、通常PL、作成者GM、他GM、admin、静的JSON由来の期待動作を整理し、バリデーション、完全削除、CASCADE、静的JSON由来、結果記録フォーマットを後続QAで使える形にまとめた。
...
ユーザー実ブラウザでの手動smoke testに備え、`docs/session-posting-rpc-smoke-manual-test.md` に簡易チェックリストと結果記録準備欄を追加した。

確認対象は、作成者GMでの作成・編集、公開状態 / 募集状態変更、ガード確認、通常PL / 他GM拒否、admin管理、完全削除、削除後の非表示、静的JSON由来の編集不可 / 削除不可、内部情報非露出、console errorなし。

M-14D-15D時点では手順支援と記録欄準備までを行った。ユーザー実ブラウザでの結果は、後続のM-14D-15Eとして成功項目、未確認項目、残課題を記録済み。

この工程ではSQL Editor実行、DB/RPC変更、フロント実装、実データ作成・更新・削除、Discord実送信、Edge Function deploy、`updates.json` 変更、commit / pushは行っていない。

## M-14D-15E 依頼書RPC smoke test結果

ユーザー実ブラウザで依頼書RPC smoke testを実施済み。結果は `docs/session-posting-rpc-smoke-manual-test.md` に記録した。

確認済み:

- 作成者GMでテスト用依頼書を新規作成・編集保存できた。
- 公開状態 / 募集状態変更と、下書き状態の公開化などのガードを確認できた。
- 通常PLではGM/admin向け管理UIが表示されない。
- 他GMでは編集不可または管理対象外として扱われる。
- adminではSupabase由来依頼書を管理対象として表示・編集できる。
- 削除専用テスト依頼書で完全削除し、削除後にcalendar / session-detail / session-post管理対象から消える。
- 静的JSON由来は編集不可 / 削除不可として扱われる。
- 画面 / DOM / console に内部情報が出ておらず、console error 0件。

未確認・残課題:

- 参加申請 / コメントを含むCASCADEの厳密確認は、必要な場合に専用データで後続確認する。
- 静的JSON fallback抑止の詳細確認、他GM拒否確認の追加精査、エラー文言の一般化確認、Discord同期状態との連動確認、静的JSON退役後の再確認は後続候補とする。

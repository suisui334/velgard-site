...
-- This SQL is a design draft for Gate MR-02.
-- It is not a reviewed apply candidate yet.
-- If promoted later, split the apply section and SELECT-only checks.
--
-- Goal:
-- - Add manual GM/admin recruitment reminder logging and RPCs.
-- - Keep this separate from public.session_reminder_logs, which is used by
--   automatic scheduled session reminders.
-- - The future Edge Function is expected to be:
--   send-session-recruitment-reminder
-- - Discord send, @everyone delivery, Edge deploy, and secret changes are
--   outside this SQL draft.
...
-- ============================================================
-- This RPC is optional for the first implementation but recommended for UI
-- disabled-state and dry-run confirmation.
--
...
-- Do not run claim/finalize RPCs in this checklist.

...
ユーザーが `docs/supabase/sql/022_gm_accepted_contacts_pc_name_apply_reviewed.sql` をSQL Editorに適用し、`get_gm_session_accepted_contacts(text)` のdrop/recreateが成功した。確認結果は `function_count = 1`、`security_definer = true`、`search_path` 設定あり、signature `get_gm_session_accepted_contacts(text)`。

戻り値列は `display_name` / `discord_handle` / `discord_mention` / `pc_name` / `pc_name_missing`。既存列 `display_name` / `discord_handle` は維持し、`pc_name` は `session_applications.pc_name_snapshot` を正とする。PC名未登録時は `PC名未登録`、DiscordユーザーID未登録・形式不正時は `登録されていません`。

権限は `authenticated EXECUTEあり`、`anon` / `public EXECUTEなし`。raw user_id / email / token は返さない。実データ投入、フロントUI実装、Discord実送信、Edge Function deploy、`updates.json` 変更は行っていない。今回CodexはSQL Editor追加実行、DB構造変更、RPC再作成、GRANT / REVOKE再実行を行っていない。

## M-15G フロント実装

GM/admin向け承認済み参加者連絡先表示で、更新済み `get_gm_session_accepted_contacts(text)` の `discord_mention` / `pc_name` / `pc_name_missing` を利用するようにした。画面表示は `Discord：discord_mention｜ユーザー名：display_name｜PC名：pc_name` のラベル付き1人1行とする。

`pc_name` は `session_applications.pc_name_snapshot` を正として表示し、`pc_name_missing = true` または空の場合は `PC名未登録` とする。Discord未登録・形式不正は `登録されていません` とし、フロント側でも不正な `discord_handle` 生値は出さない。

コピー出力も画面表示と同じラベル付き1人1行へ変更した。SQL Editor未実行、DB構造変更なし、RPC変更なし、Discord実送信なし、Edge Function deployなし。

...
Discord：<@234567890123456789>｜ユーザー名：ユーザー名｜PC名：PC名
```

DiscordユーザーIDが登録済みで形式が正しい場合は、`Discord：<@DiscordユーザーID>｜ユーザー名：ユーザー名｜PC名：PC名` の形式で出力する。DiscordユーザーIDが未登録、または形式不正の場合は、Discord欄を `Discord：登録されていません` に置き換える。

PC名が未登録の場合は、初期方針として `PC名未登録` を出すことを推奨する。リザルトテンプレート運用を考えると、PC名が未登録であることが明示される方が望ましい。

```text
Discord：<@123456789012345678>｜ユーザー名：ユーザー名｜PC名：PC名未登録
```
...
DiscordユーザーIDは `profiles.discord_handle` の17〜20桁数字から `<@ID>` を生成する。未登録または形式不正は `登録されていません` とし、生の不正値は返さない。`{{approved_call_list}}` では `Discord：<@DiscordユーザーID>｜ユーザー名：ユーザー名｜PC名：PC名`、`{{approved_pc_names}}` ではPC名一覧を使う後続方針を維持する。

`{{approved_call_list}}` は後続M-15Hで、承認済み参加者ごとに `Discord：<@DiscordユーザーID>｜ユーザー名：ユーザー名｜PC名：PC名` を1行ずつ出す。DiscordユーザーID未登録/形式不正は `Discord：登録されていません`、PC名未登録は `PC名：PC名未登録` に丸める。M-15Fではテンプレート置換処理そのものは実装しない。
...
GM/admin向け承認済み参加者連絡先表示で、RPCの `discord_mention` を優先して表示・コピーするようにした。未登録・形式不正は `登録されていません` に丸め、生の `discord_handle` 不正値は表示しない。

画面表示とコピー出力は `Discord：discord_mention｜ユーザー名：display_name｜PC名：pc_name` のラベル付き1人1行とし、後続 `{{approved_call_list}}` の原型とする。PC名未登録時は `PC名：PC名未登録`。

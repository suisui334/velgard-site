...
実装結果は `docs/supabase-session-detail-application-comment-edit-delete-ui-result.md` に分離した。当時の次工程はM-11C-3として、編集UIを `update_application_comment` に接続する段階。

2026-06-01 follow-upで、参加希望コメント一覧は `created_at` 降順の新しい順を初期表示にした。将来、参加希望コメント一覧に「新しい順 / 古い順」切替を追加する余地あり。初期表示は新しい順を基本とする。

## 20. M-11C-3実装結果追記

2026-06-01に、M-11C-3として本番 `session-detail.html` の参加希望コメント編集UIを `update_application_comment` へ接続した。

- ログイン済み、`can_edit === true`、内部 `comment_id` あり、送信中 / 保存中ではない場合だけ編集ボタンを有効化する。
- 編集ボタン押下で対象カードだけをtextarea編集モードにする。
- 保存 / キャンセルを表示し、キャンセルではRPCを呼ばない。
- 保存時は投稿時と同じ空欄 / 4000文字超過バリデーションを行う。
...
- `comment_id` は内部処理にのみ使い、画面テキストやconsoleには出さない。
- `delete_application_comment_and_maybe_cancel`、GM操作、`close_session` は呼び出していない。

実装結果は `docs/supabase-session-detail-application-comment-edit-result.md` に分離した。次工程は削除ではなく、まずユーザー実ブラウザで編集保存の確認を行う。

## 21. M-11C-4実装結果追記

2026-06-01に、M-11C-4として本番 `session-detail.html` の参加希望コメント削除UIを `delete_application_comment_and_maybe_cancel` へ接続した。

- ログイン済み、`can_delete === true`、内部 `comment_id` あり、投稿中 / 保存中 / 削除中ではない場合だけ削除ボタンを有効化する。
- 削除ボタン押下後、対象カード内に確認UIを表示する。
- 確認UIでは「最後の有効コメントを削除した場合、参加申請が取り消されることがあります。」を表示する。
- `削除する` 押下後にだけ `delete_application_comment_and_maybe_cancel(target_comment_id)` を呼び出す。
- 削除中は削除ボタン、確認ボタン、キャンセルボタンをdisabledにし、二重実行を防止する。
- 成功後はコメント一覧、申請中 / 承認済みカウント、本人申請状態を再取得する。
- 失敗時は短い安全なエラー文言だけを表示する。
- `comment_id` は内部処理にのみ使い、画面テキストやconsoleには出さない。
- GM操作、`close_session`、SQL Editor実行、DB構造変更、Codex側での削除確定実行は行っていない。

実装結果は `docs/supabase-session-detail-application-comment-delete-result.md` に分離した。次工程はユーザー実ブラウザで、複数コメント中1件削除後も申請中人数が1名のまま維持されることを確認する段階。

...
## M-14E-14K 実送信有効化前レビュー記録テンプレート
実送信有効化コード変更へ進む前に、以下を確認して記録する。実値、Webhook URL、投稿先実値、認証情報、確認対象依頼書ID相当の値、`message_preview` 本文全文は記録しない。

| 項目 | 判断 |
| --- | --- |
| 実送信対象action | `create` のみ / 未決定 |
| `dry_run = true` | preview専用維持 / 要再確認 |
| `dry_run = false` 解除範囲 | テスト用チャンネル向け `create` のみ / 未決定 |
| `update` / `close` / `delete` / `resync` | 拒否維持 / 要確認 |
| secret未設定時 | 一般化エラーで拒否 / 要確認 |
| 成功レスポンス | 最小情報のみ / 要確認 |
| Discord APIレスポンス全文 | 返さない / 要確認 |
| message id相当 | 初回は返さない / 要検討 |
| DB更新連携 | 初回は分離推奨 / 要判断 |
| 二重投稿防止 | 初回は手動1回のみ、恒久対策は後続 / 要確認 |
| ログ安全性 | 実値非露出 / 要確認 |
| 停止条件 | 確認済み / 要確認 |
...
- 実送信後にテスト用チャンネルへ1件だけ投稿されたことを確認する。
- 二重実行しない。
...
ユーザー手元で、deploy済み `sync-session-post-to-discord` に対して、テスト用チャンネル向け `create` / `dry_run = false` の初回実送信を1回だけ実施した。対象は検証用依頼書 `TEST_1`。確認対象ID相当の実値、認証情報、Supabase接続先全文、Webhook URL、投稿先実値、Discord message id相当の実値、`message_preview` 本文全文は記録しない。

実行結果:

- HTTP statusは200。
- レスポンスはJSONとしてparse成功。
...
- テスト用チャンネルに、依頼書通知用の投稿が1件作成されたことをユーザーが確認済み。
- 投稿タイトル相当として `TEST_1` の依頼書通知であることを確認済み。
- 本番募集チャンネルへの投稿はなし。
- 追加実送信は行っていない。

運用上の注意:

- 今回、PowerShellの確認入力で意図した確認語を入力していないにもかかわらず、貼り付け済みの後続行が実行され、実送信が行われた。
- 結果はテスト用チャンネルへの1件投稿に留まり、重大事故には至っていない。
- 今後の実送信系手順では、対話プロンプト依存にせず、「確認コマンド」と「送信コマンド」を明確に分離する。
- 同じ検証用依頼書での再実行は禁止する。二重投稿防止の恒久対応は、外部投稿識別子保存とDB更新連携の後続工程で扱う。

次工程候補:

1. M-14E-14R: DB更新連携設計。
2. M-14E-14S: 外部投稿識別子保存と二重投稿防止方針整理。
3. M-14E-14T: テスト用チャンネル結果を踏まえた本番募集チャンネル切り替え判断。

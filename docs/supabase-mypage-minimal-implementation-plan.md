...
## 14. A-4 Auth状態表示計画への分離

`mypage.html` にAuth状態表示を追加する前の詳細は `docs/supabase-mypage-auth-state-implementation-plan.md` に分離する。
A-4では、静的な準備中ページを壊さず、接続設定未構成時の安全フォールバック、ログイン中 / 未ログイン表示、`public_profiles.display_name` 表示の扱いを整理する。ログイン / ログアウト処理、申請一覧表示、投稿・編集・削除、GM操作はまだ扱わない。

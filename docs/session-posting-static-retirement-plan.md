...
## M-14D-14A Merge And Display Behavior

M-14D-14A時点では、`assets/js/sessionData.js` の `loadMergedSessions()` は、先に `data/sessions.json` を読み、その後Supabaseの公開行を読み込む。
...
## M-14D-14B Merge Priority Update

M-14D-14Bとして、`loadMergedSessions()` のマージ優先順位を変更した。
同じIDが存在する場合はSupabase由来を優先し、`data/sessions.json` はfallbackとして扱う。

実装では静的JSON行を先にfallbackとしてMapへ入れ、後から取得できたSupabase行を同じIDで上書きする。
静的JSON由来には `source: "static"` を付与し、Supabase由来は既存どおり `source: "supabase"` を維持する。
これにより、`session-detail.html` では同じIDのSupabase行が表示された場合に編集・削除の権限判定対象になり、静的JSONのみの予定は従来どおり編集不可・削除不可表示になる。

Supabase行は表示対象外の `visibility = hidden` や `status = draft` / `canceled` / `cancelled` でも、取得できた場合は同じIDの静的JSON fallbackを置き換える。
その後、calendar / session-detail側の既存フィルタで非表示にするため、Supabase側で非公開化・下書き化・中止化した予定が静的JSONから復活しない。
なお、RLSで取得できないSupabase行はフロント側ではID占有を判断できないため、完全退役時は静的JSON行の削除または別途公開確認が必要。

M-14D-14Bでは `data/sessions.json` はまだ削除しない。
SQL Editor未実行、DB構造変更なし、RPC変更なし、Discord実送信なし、Edge Function deployなし。
...
3. 投入後、同じIDではSupabase行が優先されることを確認し、静的JSON行はfallbackとして一時残存させるか退役するかを決める。

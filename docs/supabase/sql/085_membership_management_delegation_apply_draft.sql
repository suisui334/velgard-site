...
--   not be rendered or logged by the UI.
...
begin
  ...
  if exists (
    select 1
    from public.user_roles ur_admin
    where ur_admin.user_id = p_target_user_id
      and ur_admin.role = 'admin'
  ) then
  ...
  if not (
    ...
    or (v_current_status = 'rejected' and v_new_status in ('approved', 'pending'))
...
create or replace function public.grant_membership_manager(p_target_user_id uuid)
returns table (
  member_key uuid,
  role text,
...
begin
  ...
  values (p_target_user_id, 'membership_approver')
  on conflict (user_id, role) do nothing;

  return query
  select
    p_target_user_id as member_key,
...
create or replace function public.revoke_membership_manager(p_target_user_id uuid)
returns table (
  member_key uuid,
  role text,
  membership_status text

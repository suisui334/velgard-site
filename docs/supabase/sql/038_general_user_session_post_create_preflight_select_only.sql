...
-- Purpose: inspect why logged-in non-GM session-post make is rejected.
-- Run only after review in Supabase SQL Editor if the next gate approves it.
-- Do not paste real IDs, tokens, URLs, or user data into this file.
...
execute_privileges as (
  select
    tf.proname,
    bool_or(x.role_name = 'authenticated' and has_function_privilege(x.role_name, tf.oid, 'EXECUTE')) as authenticated_can_execute,
    bool_or(x.role_name = 'anon' and has_function_privilege(x.role_name, tf.oid, 'EXECUTE')) as anon_can_execute,
    bool_or(x.role_name = 'PUBLIC' and has_function_privilege(x.role_name, tf.oid, 'EXECUTE')) as public_can_execute
...
from (
  ...
  union all
  select
    'create_rpc_anon_public_blocked',
    case when exists (
      select 1 from execute_privileges
      where proname = 'create_session_post'
        and not anon_can_execute
        and not public_can_execute
    ) then 'ok' else 'review' end,
    coalesce((
      select 'anon=' || anon_can_execute::text || ', public=' || public_can_execute::text
      from execute_privileges
      where proname = 'create_session_post'
      limit 1
    ), 'missing'),
    'anon/PUBLIC execute state'
  union all
  select
  ...
  select
    ...
    coalesce((
      select (
        function_def ilike '%invalid_initial_status%'
      )::text
      from target_functions
      where proname = 'create_session_post'
      limit 1

-- 049_update_session_post_overload_post_apply_select_only_fix.sql
-- SELECT ONLY / DO NOT APPLY / NO MUTATION
--
-- Purpose:
-- - Fix the 048 diagnostic SQL output-column issue.
...
checks(check_name, status, result_value, note) as (
  select
    'update_session_post_overload_count'::text,
    (case when overload_count = 1 then 'ok' when overload_count > 1 then 'review' else 'missing' end)::text,
    overload_count::text,
    'expected one frontend-matching overload after 047; review if extra overloads remain'::text
  from summary_flags

  union all
  select
    'frontend_expected_arg_count'::text,
    'info'::text,
    expected_frontend_arg_count::text,
    'current renderSessionPost.js and close-mark payloads include p_session_tool'::text
  from summary_flags

  union all
  select
    'frontend_matching_overload_count'::text,
    (case when frontend_matching_overload_count = 1 then 'ok' when frontend_matching_overload_count = 0 then 'missing' else 'review' end)::text,
    frontend_matching_overload_count::text,
    'expected exactly one overload matching all frontend payload keys'::text
  from summary_flags

  union all
  select
    'frontend_matching_is_session_gm_count'::text,
    (case when frontend_matching_is_session_gm_count = 1 and frontend_matching_overload_count = 1 then 'ok' else 'review' end)::text,
    frontend_matching_is_session_gm_count::text,
    'frontend-matching overload should use owner/admin helper'::text
  from summary_flags

  union all
  select
    'frontend_matching_old_gate_count'::text,
    (case when frontend_matching_old_gate_count = 0 then 'ok' else 'review' end)::text,
    frontend_matching_old_gate_count::text,
    'nonzero means the frontend-callable overload still has the old GM-role gate'::text
  from summary_flags

  union all
  select
    'frontend_matching_authenticated_execute_count'::text,
    (case when frontend_matching_authenticated_execute_count = 1 and frontend_matching_overload_count = 1 then 'ok' else 'review' end)::text,
    frontend_matching_authenticated_execute_count::text,
    'frontend-matching overload should be executable by authenticated'::text
  from summary_flags

  union all
  select
    'frontend_matching_anon_execute_count'::text,
    (case when frontend_matching_anon_execute_count = 0 then 'ok' else 'review' end)::text,
    frontend_matching_anon_execute_count::text,
    'frontend-matching overload should not be executable by anon'::text
  from summary_flags

  union all
  select
    'old_gate_overload_count'::text,
    (case when old_gate_overload_count = 0 then 'ok' else 'review' end)::text,
    old_gate_overload_count::text,
    'nonzero means at least one update_session_post overload still has old has_role(gm)+owner logic'::text
  from summary_flags

  union all
  select
    'legacy_without_session_tool_count'::text,
    (case when legacy_without_session_tool_count = 0 then 'ok' else 'review' end)::text,
    legacy_without_session_tool_count::text,
    'legacy 13-input overload without p_session_tool should be gone after 047'::text
  from summary_flags

  union all
  select
    'anon_execute_overload_count'::text,
    (case when anon_execute_count = 0 then 'ok' else 'review' end)::text,
    anon_execute_count::text,
    'no update_session_post overload should be executable by anon'::text
  from summary_flags

  union all
  select
    'security_definer_overload_count'::text,
    (case when security_definer_count = overload_count and overload_count > 0 then 'ok' else 'review' end)::text,
    security_definer_count::text,
    'all remaining update overloads should remain security definer'::text
  from summary_flags

  union all
  select
    'search_path_overload_count'::text,
    (case when search_path_count = overload_count and overload_count > 0 then 'ok' else 'review' end)::text,
    search_path_count::text,
    'all remaining update overloads should keep search_path set'::text
  from summary_flags

  union all
  select
    'frontend_call_risk'::text,
    (case when frontend_call_risk_value = 'ready_for_owner_update_qa' then 'ok' else 'review' end)::text,
    frontend_call_risk_value::text,
    'review before edit-save or GM close-mark QA'::text
  from summary_flags

  union all
  select
    'post_apply_ready_for_owner_update_qa'::text,
    (case when post_apply_ready_for_owner_update_qa then 'ok' else 'review' end)::text,
    post_apply_ready_for_owner_update_qa::text,
    'true means SELECT-only state is ready for general-owner edit-save and close-mark QA'::text
  from summary_flags
),
overload_rows(check_name, status, result_value, note) as (
  select
    ('overload_' || overload_ordinal::text || '_signature')::text,
    'info'::text,
    signature::text,
    'signature text only; no function body or raw IDs returned'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_identity_arguments')::text,
    'info'::text,
    identity_arguments::text,
    'input argument names/types/defaults; no function body returned'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_input_arg_count')::text,
    'info'::text,
    input_arg_count::text,
    'frontend update payload expects all listed named parameters'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_matches_frontend_payload')::text,
    (case when matches_frontend_payload then 'ok' else 'review' end)::text,
    matches_frontend_payload::text,
    'true means all current frontend update payload keys are present'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_has_p_session_tool_arg')::text,
    (case when has_p_session_tool_arg then 'ok' else 'review' end)::text,
    has_p_session_tool_arg::text,
    'current frontend update payload includes p_session_tool'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_has_old_gm_owner_gate')::text,
    (case when has_old_gm_owner_gate then 'review' else 'ok' end)::text,
    has_old_gm_owner_gate::text,
    'true means old role-gated owner logic remains in this overload'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_has_is_session_gm_pattern')::text,
    (case when has_is_session_gm_pattern then 'ok' else 'review' end)::text,
    has_is_session_gm_pattern::text,
    'true means owner/admin helper appears in this overload'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_security_definer')::text,
    (case when security_definer then 'ok' else 'review' end)::text,
    security_definer::text,
    'security definer should remain true'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_has_search_path')::text,
    (case when has_search_path then 'ok' else 'review' end)::text,
    has_search_path::text,
    'search_path should remain configured'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_authenticated_execute')::text,
    (case when authenticated_can_execute is true then 'ok' else 'review' end)::text,
    coalesce(authenticated_can_execute::text, 'role_missing')::text,
    'authenticated should execute the frontend-callable RPC'::text
  from function_checks

  union all
  select
    ('overload_' || overload_ordinal::text || '_anon_execute')::text,
    (case when anon_can_execute is false then 'ok' else 'review' end)::text,
    coalesce(anon_can_execute::text, 'role_missing')::text,
    'anon should not execute update_session_post'::text
  from function_checks
),
final_rows(check_name, status, result_value, note) as (
  select check_name, status, result_value, note
  from checks

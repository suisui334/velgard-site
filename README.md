...
- `docs/`
  ...
  - `docs/supabase-step10-local-auth-smoke-test.md`: Supabase Step 10 ローカルAuth文脈RLSスモークテスト手順
  - `docs/supabase-step11-rls-smoke-fix-plan.md`: Supabase Step 11 RLS smoke test FAIL修正計画
  ...
  - `docs/session-type-field-result.md`: M-13A sessionType分類フィールド 実装結果
  - `docs/session-application-deadline-field-result.md`: M-13B 申請締切日時フィールド 実装結果
  ...
  - `docs/supabase-f3-auth-state-prototype.md`: Supabase F-3 dev ログイン状態表示プロトタイプ手順
  - `docs/supabase-f4-application-comment-prototype.md`: Supabase F-4 dev 参加希望コメント投稿プロトタイプ手順
  ...
  - `docs/supabase-session-detail-application-withdraw-history-plan.md`: Supabase M-11D 申請辞退 / 申請履歴フロー 調査・設計
  - `docs/supabase-session-detail-application-withdraw-rpc-plan.md`: Supabase M-11D-2 本人申請辞退RPC SQL草案 設計
  - `docs/supabase-session-detail-application-withdraw-rpc-result.md`: Supabase M-11D-2 本人申請辞退RPC SQL適用結果
...
- Supabase Step 10 ローカルAuth文脈RLSスモークテスト手順は `docs/supabase-step10-local-auth-smoke-test.md`、検証スクリプトは `scripts/supabase-rls-smoke-test.mjs` に分離済み。本番サイトへ組み込まず、`.env.local` の実値はGit管理しない。
...
- `docs/supabase-mypage-display-name-sql-execution-review-plan.md`: M-9 SQL実行前レビューとして、009草案の実行範囲、事前確認SQL、trigger / backfill / RPC確認点、実行順序、ロールバック、実行後検証、RLS smoke test更新要否を整理した。この工程でもSQL実行、`mypage.html` 実装、`assets/js/mypageAuthClient.js` 変更は行っていない。
- `docs/supabase-mypage-display-name-sql-result.md`: M-9 display_name DB反映結果として、`handle_new_auth_user_profile` と `update_display_name(new_display_name text)` の存在、`update_display_name` の anon execute不可 / authenticated execute可、`public_profiles` が `id` / `display_name` のみ、`auth_users_without_profile = 0` を確認済みとして整理した。追加SQLはこれ以上実行せず、次工程は `mypage.html` のdisplay_name表示・編集フロント実装とする。
- M-9 display_nameフロント最小実装として、`mypage.html` のログイン済みアカウント機能内に表示名表示・編集を追加した。`public_profiles` から `display_name` を取得し、`update_display_name` RPCで本人の表示名を保存する。email / user_id / token は画面に出さず、申請一覧・参加予定・`session-detail.html` 統合は未実装のまま。
- M-9修正として、新規登録時に `display_name` 入力欄を追加し、`signUp` のuser metadataへ渡すようにした。既存ユーザーは `update_display_name` RPCでマイページから表示名を保存する。email / user_id / token は画面に出さない。
...
- M-11Aとして、`session-detail.html` の参加希望コメント欄を読み取り専用表示へ更新した。`get_public_session_comments` と `get_public_session_application_counts` で公開コメント一覧と公開カウントを取得し、表示名、本文、申請状態、投稿/編集日時、申請中人数、承認済み人数を表示する。`参加希望人数` と `キャンセル待ち` はM-11Aの画面には表示しない。申請締切はM-13Bで `data/sessions.json` の `applicationDeadline` 明示フィールドとして追加済みで、`startTime` / `endTime` は流用しない。投稿、編集、削除、GM操作、`close_session`、SQL実行、DB変更は行っていない。実装結果は `docs/supabase-session-detail-application-comments-read-result.md` に整理済み。
- M-11A follow-upとして、`session-detail.html` の表示順を基本情報、概要、補足情報、参加希望コメントへ整理し、概要をカード状ブロック表示にした。`鉄道` / `調査` のような自由タグはsession-detailでは表示しない。セッション種別はM-13Aで `sessionType` 明示フィールドとして追加済みで、候補は `単発シナリオ`、`キャンペーン`、`特殊`、`その他` として扱う。
...
- M-13Aとして、`data/sessions.json` の各セッションに固定分類 `sessionType` を追加した。既存7件は `one-shot` とし、表示名は `単発シナリオ`。`session-detail.html` の基本情報と `calendar.html` の選択日セッション一覧に種別を表示する。自由タグは分類に使わず、session-detailで自由タグを表示しない方針も維持している。SQL Editor実行、DB変更、`updates.json` 変更は行っていない。実装結果は `docs/session-type-field-result.md` に整理済み。
- M-13Bとして、`data/sessions.json` の各セッションに `applicationDeadline` を追加した。既存7件は開催日前日の `23:59` で統一し、`session-detail.html` の基本情報と `calendar.html` の選択日セッション一覧に `申請締切` として表示する。`startTime` / `endTime` は開催時刻のまま扱い、申請締切には流用していない。SQL Editor実行、DB変更、`updates.json` 変更は行っていない。実装結果は `docs/session-application-deadline-field-result.md` に整理済み。
